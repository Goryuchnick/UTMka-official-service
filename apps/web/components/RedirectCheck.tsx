...
import { explainFailure, UTM_PARAM_NAMES, type RedirectReport } from '@utmka/core'
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      if (typeof data === 'object' && data !== null && 'report' in data) {
        setReport(data.report as RedirectReport)
      } else {
        const message =
          typeof data === 'object' && data !== null && 'error' in data ? String(data.error) : ''
        setError(message || 'Не получилось проверить')
      }
    } catch {
