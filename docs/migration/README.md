...
| [STEP_4_MACOS.md](STEP_4_MACOS.md) | Этап 4: macOS сборка | ✅ Выполнено |
...
- [x] Создан PyInstaller spec для macOS (`installers/macos/UTMka.spec`)
- [x] Создан скрипт сборки с автосинхронизацией версии (`installers/macos/build.py`)
- [x] Создан скрипт создания DMG (с fallback на hdiutil)
- [x] Создан скрипт подписи и нотаризации (`installers/macos/sign_and_notarize.sh`)
- [x] Поддержка обеих архитектур (arm64 и x86_64)
- [x] Интеграция с OTA обновлениями (уже реализовано в `src/core/updater.py`)
- [x] Документация по сборке (`installers/macos/README.md`)

**Использование:**
```bash
python installers/macos/build.py
# Результат: dist/UTMka.app и dist/UTMka-2.2.0-macOS.dmg
```
