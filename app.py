...
filedialog = None
IS_HEADLESS = False
from flask import Flask, request, jsonify, render_template, send_from_directory, send_file, make_response, Response
...
def index():
    ...
    # Отключаем автоматическую загрузку favicon браузером
    response.headers['X-Favicon'] = 'none'
    # Добавляем заголовки для быстрой загрузки
...
def favicon():
    """Отдает favicon. Быстрый ответ без блокировки."""
    try:
        favicon_path = resource_path('logo/logoutm.png')
        if not os.path.exists(favicon_path):
            # Если файл не найден, возвращаем пустой ответ немедленно
            return Response(status=204)  # No Content
        
        # Используем кэширование для быстрой отдачи
        response = send_file(favicon_path, mimetype='image/png')
        response.cache_control.max_age = 31536000  # 1 год
        response.cache_control.public = True
        return response
    except Exception as e:
        # Если произошла ошибка, возвращаем пустой ответ, чтобы не блокировать
        return Response(status=204)  # No Content
...
def get_history():
    ...
    try:
        # Оптимизация: быстрые настройки для запроса
        conn.execute('PRAGMA busy_timeout = 1000')  # 1 секунда вместо 3
        conn.execute('PRAGMA synchronous = NORMAL')
...
def get_templates():
    ...
    try:
        ...
        templates = conn.execute(
            'SELECT id, user_email, name, utm_source, utm_medium, utm_campaign, utm_content, utm_term, tag_name, tag_color, created_at FROM templates WHERE user_email = ? ORDER BY created_at DESC LIMIT 500',
            (user_email,)
        ).fetchall()
...
if __name__ == '__main__':
    ...
    # Пытаемся найти свободный порт, начиная с 5000
    ...
    # Проверяем порт 5000 и ищем свободный, если занят
    flask_port = 5000
    if is_port_in_use(5000):
        print("Порт 5000 занят, ищем свободный порт...")
        free_port = find_free_port(5000, 10)
        if free_port:
            flask_port = free_port
            print(f"Используем порт {flask_port} вместо 5000")
        else:
            print("Не удалось найти свободный порт. Закройте другие экземпляры приложения.")
            sys.exit(1)
    ...
    def run_flask():
        try:
            ...
            if "Address already in use" in str(e):
                print(f"Порт 5000 уже занят. Попробуйте закрыть другие экземпляры приложения.")
    ...
    max_attempts = 50  # Увеличено количество попыток
    for i in range(max_attempts):
        try:
            response = requests.get(f'http://127.0.0.1:{flask_port}', timeout=2)
            if response.status_code == 200:
                print("Сервер запущен успешно!")
                # Даем серверу немного времени на полную инициализацию
                time.sleep(0.5)
                break
        except Exception as e:
            if i % 10 == 0:  # Выводим сообщение каждые 10 попыток
                print(f"Ожидание запуска сервера... ({i+1}/{max_attempts})")
        time.sleep(0.2)  # Увеличена задержка между попытками
    else:
        print("Не удалось запустить сервер за", max_attempts * 0.2, "секунд!")
        exit(1)
    ...
    try:
        ...
        print("---")
        print("КРИТИЧЕСКАЯ ОШИБКА: Не удалось запустить графический интерфейс (PyWebView).")
        print("Для работы приложения в нативном режиме требуется Microsoft Edge WebView2.")
        print("Пожалуйста, установите WebView2 по ссылке: https://developer.microsoft.com/en-us/microsoft-edge/webview2/")
        print("---")
