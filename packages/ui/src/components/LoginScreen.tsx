...
import { backendMessage, hasMixedScripts } from '@utmka/core'
...
export function LoginScreen() {
  ...
  const login = useCallback(async () => {
    ...
    setStage('busy')
    try {
      await backend.account!.login(phrase)
      setAccount('member')
      nav.go('/templates')
    } catch (error) {
  ...
  const register = useCallback(async () => {
    ...
    setStage('busy')
    try {
      const { passphrase } = await backend.account!.register()
      setFresh(passphrase)
      setAccount('member')
      setStage('fresh')
    } catch (error) {
    ...
    return (
      <div className="screen-scroll">
        <div className="glass">
          ...
          <div className="result-row">
            <button type="button" className="btn btn--sm" onClick={() => nav.go('/templates')}>
              <PixelIcon name="star" />
              Шаблоны
            </button>
            <button type="button" className="btn btn--sm" onClick={() => nav.go('/history')}>
